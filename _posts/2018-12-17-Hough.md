...
<script>
	var htSample1 = {
		...
		update_hough_pixels: function() {
			var maxA = Math.max.apply(Math, this.accumulator);
			if (maxA == 0.0) {
				// TODO
				return;
			}
			for (var i=0; i<this.accumulator.length; i+=1) {
				var j = i*4;
				var v = Math.floor(255*this.accumulator[i]/maxA);
		...
		update_hough_lines: function(x0, y0) {
			...
			for (var y=0; y<this.aSize.y; y+=1) {
				for (var x=0; x<this.aSize.x; x+=1) {
					if (this.accumulator[i] >= (maxA*0.8)) { // TODO: tune this
